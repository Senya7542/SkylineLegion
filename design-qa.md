...
## v5.17 命中反馈、Boss 落点和路线数据

- 来源反馈：当前数值体验满意，进入展示版 polish：命中/死亡反馈更爽，Boss 炮弹落点更清晰，后台记录玩家走门路线与胜负结果。
- 实现地址：`http://127.0.0.1:4180/`
- 自动验证截图：`output/playwright/v5.17-polish.png`。

### 修复内容

- [Fixed P1] ImpactPool 支持按事件传入颜色，普通命中、击杀、重炮群杀、Boss 爆点可以使用不同冲击色和尺寸。
- [Fixed P1] 敌人击杀反馈增强：击杀时使用更亮的青白爆闪和更大的扩散环，重炮击杀会产生更大的群体冲击。
- [Fixed P1] Boss 落点提示增强：原有危险圈之外增加收缩外环，随炮弹飞行逐步逼近真实杀伤范围。
- [Fixed P2] 新增 `getGateRouteSummary()`，按每道门是否选择较优值统计 `betterChoices / worseChoices / quality`。
- [Fixed P2] `balanceLog`、`window.__SKYLINE_DEBUG__` 和 devPanel 都带路线摘要，便于后续验证“走对门胜利、走错门失败”的调参目标。

### 验证

- 逻辑测试：`node --test tests/combatLogic.test.mjs` 通过 `20/20`。
- 生产构建：`npm run build` 通过；Vite 仍然只报已有的大 vendor chunk 警告。
- Chrome 自动局：`?flags=autoPilot,noAssets,devPanel` 完整胜利，页面错误为 `0`。
- 自动局关键数据：最终 `78` 兵，Boss 时长约 `5.49s`，路线统计 `perfect`，较优门 `4`，较差门 `0`。

### 后续建议

- P1: 进入展示录屏前，可以专门做一轮“失败路线”自动验证，确保走错门失败的后台数据也能稳定记录。
- P2: 如果要继续提升买量感，可以加轻量音效占位或屏幕边缘震动节奏，但别再动当前核心数值。

## v5.16 敌群中路空洞补位

- 来源反馈：当前数值手感已经接近目标；剩余玩法 bug 是敌方小兵推进时，中间敌人被打掉后，两侧剩余敌人没有朝中心补上。
- 实现地址：`http://127.0.0.1:4180/`

### 修复内容

- [Fixed P1] 敌群物理目标不再只固定到每个敌人的出生 home 点；现在会按当前存活数量缩小队形，并围绕存活敌人的 home 中心重新居中。
- [Fixed P1] 当中路敌人死亡后，左右两侧存活敌人的目标点会向中心收缩，配合原有碰撞分离形成自然补位。
- [Fixed P2] 保留完整敌群时的圆形分布：只有随着存活数量下降，队形才逐步变紧，不影响满员波次的展开感。

### 验证

- 逻辑测试：新增“中间敌人死亡后左右敌人向中心补位”回归测试，`node --test tests/combatLogic.test.mjs` 通过 `19/19`。
- 生产构建：`npm run build` 通过；Vite 仍然只报已有的大 vendor chunk 警告。

## v5.15 敌方波次数量回调和最终门补偿

- 来源反馈：敌人波次从上一版 `180 / 240 / 300 / 360` 回调为 `180 / 200 / 220 / 240`，最后一对门在当前数值基础上整体 `+30`。
- 实现地址：`http://127.0.0.1:4180/`

### 修复内容

- [Fixed P1] 敌方四波数量调整为 `180 / 200 / 220 / 240`，保留圆形群集分布和现有速度。
- [Fixed P1] 最后一对门整体上调 `+30`：较差侧从 `-88~-70` 改为 `-58~-40`，较好侧从 `-60~-48` 改为 `-30~-18`。
- [Fixed P2] 回归测试同步覆盖新波次数量和最终门范围，避免后续调参误回退。

### 验证

- 逻辑测试：`node --test tests/combatLogic.test.mjs` 通过 `18/18`。
- 生产构建：`npm run build` 通过；Vite 仍然只报已有的大 vendor chunk 警告。

## v5.14 敌群数量和圆形分布验证

- 来源反馈：玩家反馈最后两波敌方小兵过密、过夸张；指定敌人数量改为 `180 / 240 / 300 / 360`，并要求敌人不要被限制在方形范围内，而是像我方小兵一样围绕中心做圆形物理群集。
- 实现地址：`http://127.0.0.1:4180/`
- 自动验证截图：`output/playwright/v5.14-round-enemies.png`。

### 修复内容

- [Fixed P1] 敌方波次数量按指定值调整为 `180 / 240 / 300 / 360`。
- [Fixed P1] 敌人生成从 `x/z` 分别 clamp 的方形范围改为按波次数量归一化的圆盘分布，围绕群集中心展开。
- [Fixed P1] 移除敌群物理更新后的矩形裁切，避免大波次被硬压进方形范围。
- [Fixed P2] 保留大波次物理性能保护：敌人很多时降低分离迭代次数，小波次仍保持更高分离精度。
...
- 自动局关键数据：Boss 前 `287` 兵，最终 `280` 兵，Boss 时长约 `8.28s`。
- 敌群间距：四波最小间距约 `0.37-0.38`，说明圆形分布下仍保持物理分离，没有挤成单块。

### 后续建议

- P1: 圆形分布后视觉更自然，但压制力下降。下一轮如果还要压兵量，优先提高第 3/4 波速度或拆成前后两段，而不是继续把数量堆得过密。

## v5.13 敌群数量压制验证

- 来源反馈：玩家希望用敌方小兵数量对抗我方小兵，每波都能清掉我方大部分小兵，同时过门又能补回一批，形成“补充-消耗-再补充”的节奏。
- 实现地址：`http://127.0.0.1:4180/`
- 自动验证截图：`output/playwright/v5.13-enemy-pressure.png`。

### 修复内容

- [Fixed P1] 敌方波次数量从 `44 / 72 / 112 / 144` 提升到 `112 / 240 / 420 / 620`，用数量形成明显压制。
- [Fixed P2] 大规模敌群前排更靠前，新增敌人更容易进入接触区，而不是全部挤在后排被远程火力清掉。
- [Fixed P2] 大规模敌群降低分离迭代次数，减少 400+ 敌人时的物理计算压力；小规模敌群仍保持原来的 5 次分离。

### 验证

- 逻辑测试：`node --test tests/combatLogic.test.mjs` 通过 `17/17`。
- 生产构建：`npm run build` 通过；Vite 仍然只报已有的大 vendor chunk 警告。
- 浏览器自动局：`?flags=autoPilot,noEffects,noAssets` 完整胜利，页面错误为 `0`。
- 自动局关键数据：波次接触总损失 `271`，Boss 前 `119` 兵，最终 `96` 兵，Boss 时长约 `10.03s`。
- 自动局门结果：第 1-4 门分别约为 `+58 / +457 / +397 / +96`，虽然门值很高，但敌群数量已能把兵力从上限压回到两位数。

### 后续建议

- P1: 下一轮可以不再继续堆数量，而是下调前两门成长或给前两门更负的初始值，避免第二门直接顶满后全靠敌人硬消耗。
- P2: 如果手玩感觉画面太挤，可以把第 4 波拆成两段连续小波，保持压制但视觉更清楚。

## v5.12 区域内倍增门命中验证

- 来源反馈：玩家明确要求不是视觉上接近时才命中门，而是只要当前区域对应的倍增门已经是下一道未解决门，子弹从进入该区域开始就能打中；进入下一区域也同理。
- 实现地址：`http://127.0.0.1:4180/`
- 自动验证截图：`output/playwright/v5.12-region-gate.png`。

### 修复内容

- [Fixed P1] 移除门碰撞里的视觉距离阈值。现在门命中只看“下一道未解决门”以及“是否进入该门区域”，不再要求门接近玩家。
- [Fixed P1] 回归测试覆盖当前区域和下一区域：区域开始后，当前门即使还在远处也能被同区域子弹命中。
- [Fixed P2] 保留未来门保护：尚未进入对应区域的后续门不会提前吃子弹。

### 验证

- 逻辑测试：`node --test tests/combatLogic.test.mjs` 通过 `16/16`。
- 生产构建：`npm run build` 通过；Vite 仍然只报已有的大 vendor chunk 警告。
- 浏览器自动局：`?flags=autoPilot,noEffects,noAssets` 完整胜利，页面错误为 `0`。
- 自动局门结果：第 1-4 门分别约为 `+46 / +322 / +302 / +153`，证明区域开始命中已生效。
- 平衡观察：由于整段区域都能刷门，兵量回到接近上限，Boss 前 `320` 兵，最终 `316` 兵。

### 后续建议

- P1: 当前命中逻辑符合区域规则后，下一轮需要重新按“整段区域刷门”调数值，优先下压前两门成长或增加前两波敌人消耗，否则第二门开始就会把兵量顶满。

## v5.11 门阻挡距离回调验证

- 来源反馈：玩家反馈上一版距离门稍远时子弹打不到门，门阻挡窗口收得太近。
- 实现地址：`http://127.0.0.1:4180/`
- 自动验证截图：`output/playwright/v5.11-gate-window.png`。

### 修复内容

- [Fixed P1] 门阻挡窗口从 `z >= -2.25` 前移到 `z >= -5.6`，让门明显接近玩家时就能挡住子弹。
- [Fixed P1] 新增回归测试覆盖两个边界：`z=-5` 可以命中当前门，`z=-9` 仍然不会过早刷远处门。
...
- 生产构建：`npm run build` 通过；Vite 仍然只报已有的大 vendor chunk 警告。
- 浏览器自动局：`?flags=autoPilot,noEffects,noAssets` 完整胜利，页面错误为 `0`。
- 自动局门结果：第 1-4 门分别约为 `+24 / +88 / +52 / +15`，没有回到几百的异常门值。
- 自动局 Boss 结果：Boss 前 `180` 兵，最终 `179` 兵，Boss 时长约 `6.82s`。

### 后续建议

- P1: 这版门更早挡子弹后，兵量又回升到 `180` 左右；如果手玩仍偏强，下一轮优先继续下调后两门基础值或加厚最后一波敌人。

## v5.10 子弹寿命、门窗口和后段平衡验证

- 来源反馈：玩家反馈子弹到一定距离会自己消失、不自然；我方整体过强；希望增加敌方小兵群集数量，后两门在旧数值基础上整体 `-40`，Boss 前敌人更多，Boss 炮弹更快。
- 实现地址：`http://127.0.0.1:4180/`
- 自动验证截图：`output/playwright/v5.10-balance-smoke.png`。

### 修复内容

- [Fixed P1] 玩家普通子弹和重炮不再用短寿命在半空裁掉，改为足够长的寿命，正常由命中或离屏回收。
- [Fixed P1] 长寿命子弹会导致远距离提前刷门。门碰撞现在只允许下一道未解决门在接近玩家、成为当前阻挡物后吃子弹，避免第二门开始异常涨到数百。
- [Fixed P1] 后两门按旧模板整体下移 `40`，仍然保留两边差异：第三门约 `-64~-54` 对 `-16~-6`，第四门约 `-88~-70` 对 `-60~-48`。
- [Fixed P2] 敌群数量从 `40 / 60 / 86 / 96` 提到 `44 / 72 / 112 / 144`，重点增强 Boss 前最后一波。
- [Fixed P2] Boss 炮弹速度从 `8.2` 提到 `11.4`，保留最前方小兵索敌和群体伤害。

### 验证

- 逻辑测试：`node --test tests/combatLogic.test.mjs` 通过 `15/15`。
- 生产构建：`npm run build` 通过；Vite 仍然只报已有的大 vendor chunk 警告。
- 浏览器自动局：`?flags=autoPilot,noEffects,noAssets` 完整胜利，页面错误为 `0`。
- 自动局门结果：第 1-4 门分别为 `+16 / +49 / +33 / -17`，不再出现 `+300` 级别的异常门值。
- 自动局 Boss 结果：Boss 前 `81` 兵，最终 `76` 兵，Boss 时长约 `7.69s`，Boss 炮弹单次命中损失 `4`。

### 后续建议

- P1: 继续手动试玩这版门窗口，重点看“门太晚挡子弹”是否会被感觉到；如果晚，可以略微提前窗口，同时加大后段负值。
- P2: 如果仍然觉得我方偏强，优先增加最后一波敌人的接触伤害/密度，而不是再减少射手或做显眼上限。
